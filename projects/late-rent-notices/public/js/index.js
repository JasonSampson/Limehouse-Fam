    (async function () {
      ...
      subtabNotices.addEventListener("click", (e) => {
        e.preventDefault();
      ...
      function openDurationClass(openedAt) {
        const opened = new Date(openedAt);
        if (Number.isNaN(opened.getTime())) return "";
        const days = Math.floor((Date.now() - opened.getTime()) / (1000 * 60 * 60 * 24));
      ...
      async function getPmUsers() {
        if (!pmUsersCache) {
          const { pmUsers } = await LimehouseAPI.get("/api/pm-users");
          pmUsersCache = pmUsers;
        }
        return pmUsersCache;
      }
